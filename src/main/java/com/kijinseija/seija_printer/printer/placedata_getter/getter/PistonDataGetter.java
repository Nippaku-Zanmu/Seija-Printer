...
public class PistonDataGetter extends AbstractDataGetter {
    ...
    public PlaceData getData(BlockPos pos, BlockState needState, List<Direction> dirs) {
        for (Direction dir : dirs) {
            ...
            Direction opposite = BlockUtil.getEntityFacingOrder((float) SeijaUtil.getYaw(hitVec), (float) SeijaUtil.getPitch(hitVec))[0].getOpposite();
            if (opposite.toString().toLowerCase().equals(needState.get(FacingBlock.FACING).toString()))
